...
- Idempotency problem when a comment follows a multiline infix expression in a let binding. A comment on its own line after `c +` and `d`, at the column of both, was attached to `d` rather than to `c + d`. That made the expression multiline, so the operator moved to the start of the next line, and on the next format the comment went to the whole expression and it was joined onto one line again. An indented comment now goes after the outermost preceding node that starts a line at its column, and only falls back to a node further along a line when there is none. [#3488](https://github.com/fsprojects/fantomas/issues/3488)
...
- Breaking: the `.fantomasignore` that applies to a file is found by walking up from that file, rather than from the directory Fantomas was started in. This is what the daemon has always done, and of everything in this release it is the change most likely to be noticed. Where you run the command from no longer decides which ignore file applies: `cd tools && fantomas ../src` used to match files under `src` against `tools/.fantomasignore`, and could skip every one of them on the strength of an ignore file that had nothing to do with them, while never reading the one sitting above `src`. In the other direction, an ignore file in a subfolder was honoured by an editor and invisible to a pipeline, so the same file was skipped in one and formatted in the other; a nested ignore file that used to have no effect on a command line run now has one. Fantomas still reads one ignore file per file, the nearest at or above it, and does not merge in the ones above that. [#3416](https://github.com/fsprojects/fantomas/pull/3416)

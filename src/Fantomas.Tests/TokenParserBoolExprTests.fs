           ...
           (BoolExpr.Not
               (BoolExpr.Or
                   (BoolExpr.Ident "INTERACTIVE",
                    BoolExpr.Or
                        (BoolExpr.Not <| BoolExpr.And(BoolExpr.Ident "FOO", BoolExpr.Ident "BAR"), BoolExpr.Ident "BUZZ"))))
...
let ``BoolExpr merge``() =
    let getSource es =
        es
        |> Seq.map (fun e -> (sprintf "#if %s" e) + System.Environment.NewLine + "#endif")
...
type BoolExprGenerator =
    static member SimpleIdent() =
        { new Arbitrary<string>() with
            member x.Generator = Gen.choose (int 'A', int 'Z') |> Gen.map (char >> string)
        ...
        ({ verboseConf with Arbitrary = [ typeof<BoolExprGenerator> ] }, (
             fun es ->
                 ...
                 let definesToLiterals ds =
                     ...
                     allDefines
                     |> List.map (fun x ->
                         if Set.contains x s then BoolExpr.Positive x
                         else BoolExpr.Negative x)
                 es
                 |> List.collect (fun e ->
                     [ e
                       BoolExpr.Not e ])
                 ...
                 |> List.forall
                     ...
                     defines
                     |> List.exists
                         (fun ds -> BoolExpr.toFlatCNF e |> fun cnf -> BoolExpr.eval cnf (definesToLiterals ds)))))

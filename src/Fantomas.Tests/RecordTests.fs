...
[<Test>]
let ``record with bracketOnSeparateLine`` () =
...
[<Test>]
let ``instance of record with bracketOnSeparateLine`` () =
...
[<Test>]
let ``nested record with bracketOnSeparateLine`` () =
    formatSourceString false """let myRecord =
    { Level = 1
      Progress = "foo"
      Bar = "bar"
...
[<Test>]
let ``update record with bracketOnSeparateLine`` () =
    formatSourceString false """let myRecord =
    { myOldRecord
        with Level = 2
             Bar = "barry"
...
[<Test>]
let ``anonymous record with bracketOnSeparateLine`` () =
...
[<Test>]
let ``anonymous type with bracketOnSeparateLine`` () =
...
[<Test>]
let ``records in list with bracketOnSeparateLine`` () =
...
[<Test>]
let ``record as parameter to function with bracketOnSeparateLine`` () =
    formatSourceString false """let configurations =
...
[<Test>]
let ``anonymous records in list with bracketOnSeparateLine`` () =
    formatSourceString false """let configurations =
    [
        {| Build = true; Configuration = "RELEASE"; Defines = ["FOO"] |}
        {| Build = true; Configuration = "DEBUG"; Defines = ["FOO";"BAR"] |}

        ...
        | ModuleDecl.TopLevelBinding bindingNode ->
            ...
            let genFunctionName =
                ...
                | Choice1Of2 functionNameNode ->
                    ...
                    | SingleItemImmutableArray(IdentifierOrDot.Ident node) -> !-node.Text
                    | _ -> !- "error"
                | Choice2Of2 _ -> !- "error"
...
"""
    ...
    let f (tree: Oak) =
        ...
        | TwoItemsImmutableArray(ModuleDecl.TopLevelBinding a, ModuleDecl.TopLevelBinding b) ->

            let genBinding (node: BindingNode) =
                let name =
                    match node.FunctionName with
                    | Choice1Of2 iln ->
                        match iln.Content.[0] with

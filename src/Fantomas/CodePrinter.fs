...
and genVal(Val(ats, _, ao, s, t, vi, _)) = 
    let (FunType ts) = (t, vi)
    colPost sepNln sepNone ats genAttribute 
...
and genMemberSig = function
    | MSMember(Val(ats, _, ao, s, t, vi, _), mf, r) -> 
        let (FunType ts) = (t, vi)
        genCommentsAt r +> colPost sepSpace sepNone ats genAttribute 

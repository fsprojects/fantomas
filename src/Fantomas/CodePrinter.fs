...
and genOnelinerAttributes astContext (ats: SynAttributes) =
    let ats = ats |> List.collect (fun a -> a.Attributes)
...
and genSynBindingFunction
    ...
    (px: FSharp.Compiler.Syntax.PreXmlDoc)
    (ats: SynAttributes)
...
and genConst (c: SynConst) (r: Range) =
    ...
    | SynConst.Measure (c, _, m) ->
        ...
        let genNumber (ctx: Context) =
            ...
            | SynMeasure.Seq (_, mr) ->
                let numberRange =
                    ctx.MkRange r.Start (Position.mkPos mr.StartLine (mr.StartColumn - 1))

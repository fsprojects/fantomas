...
let rec (|UppercaseExpr|LowercaseExpr|) (expr: Expr) =
    ...
    let lastFragmentInList (identList: IdentListNode) =
        match Seq.tryLast identList.Content with
        | None
    ...
    | Expr.Chain node ->
        match Seq.tryLast node.Links with
        | None
...
let genAttributesCore (ats: AttributeNode immarray) =
...
let genOnelinerAttributes (n: MultipleAttributeListNode option) =
    ...
    | Some n ->
        let ats =
            ImmutableArray.collect (fun (al: AttributeListNode) -> al.Attributes) n.AttributeLists

        let openingToken =
            ImmutableArray.tryHead n.AttributeLists
            |> Option.map (fun (a: AttributeListNode) -> a.Opening)

        let closingToken =
            ImmutableArray.tryLast n.AttributeLists
...
let genExpr (e: Expr) =
    ...
    | Expr.CompExprBody node ->
        ...
        | TwoItemsImmutableArray(ComputationExpressionStatement.LetOrUseStatement letOrUseNode,
    ...
    | Expr.PrefixApp node ->
        ...
        | Expr.App appNode ->
            ...
            | SingleItemImmutableArray(Expr.Constant(Constant.Unit _) as argExpr) ->
                genSingleTextNode node.Operator
                +> sepSpace
                +> genExpr appNode.FunctionExpr
                +> genExpr argExpr
    ...
    | Expr.SameInfixApps node ->
        ...
        let multilineExpr =
            ...
            | EmptyImmutableArray -> genExpr node.LeadingExpr
            | SingleItemImmutableArray(operator, e2) ->
                // Impossible to reach..
                let m =
            ...
            | HeadAndTailInImmutableArray((operator, e2), es) ->
                let m =
                    FSharp.Compiler.Text.Range.unionRanges (Expr.Node node.LeadingExpr).Range (Expr.Node e2).Range
    ...
    | Expr.Chain node ->
        let genLink (isLastLink: bool) (link: ChainLink) =
            ...
            | ChainLink.Expr expr ->
                ...
                | Expr.App appNode ->
                    match appNode.Arguments with
                    | SingleItemImmutableArray(Expr.ArrayOrList _ as arrayOrList) ->
        ...
        let long =
            ...
            let (|LeadingSimpleChain|_|) (links: ChainLink immarray) =
                ...
                |> (fun _ ->
                    if Seq.isEmpty leading then
                        ...
                        Some(ImmutableArray.ofSeq leading, ImmutableArray.ofSeq rest))

            let rec genIndentedLinks (lastLinkWasSimple: bool) (links: ChainLink immarray) (ctx: Context) : Context =
                match links with
                | EmptyImmutableArray -> ctx
                | TwoItemsImmutableArray(ChainLink.Dot dot, link) ->
                    processLink lastLinkWasSimple dot link ImmutableArray.empty ctx
                | HeadAndTailInImmutableArray(ChainLink.Dot dot, HeadAndTailInImmutableArray(link, rest)) ->
                    processLink lastLinkWasSimple dot link rest ctx
                | _ -> failwith "Expected dot in chain at this point"

            and processLink lastLinkWasSimple dot link rest ctx =
            ...
            let genFirstLinkAndIndentOther (firstLink: ChainLink) (others: ChainLink immarray) =
                genLink false firstLink +> indentSepNlnUnindent (genIndentedLinks false others)

            match node.Links with
            | EmptyImmutableArray -> sepNone
            | SingleItemImmutableArray item -> genLink true item
                    ...
                    fun ctx ->
                        isShortExpression
                            ...
                            (match leadingChain with
                             | EmptyImmutableArray -> sepNone
                             | SingleItemImmutableArray link -> genLink true link
                             | HeadAndTailInImmutableArray(head, links) ->
                                 genLink false head +> indent +> genIndentedLinks true links +> unindent)
                            ctx
                | _ ->
                    expressionFitsOnRestOfLine
                        ...
                        (match leadingChain with
                         | EmptyImmutableArray -> sepNone
                         | SingleItemImmutableArray link -> genLink true link
                         | HeadAndTailInImmutableArray(head, rest) ->
                             genLink false head +> indentSepNlnUnindent (genIndentedLinks false rest))
                    +> indentSepNlnUnindent (genIndentedLinks false links)

            | HeadAndTailInImmutableArray(head, links) -> genFirstLinkAndIndentOther head links
    ...
    | Expr.AppWithLambda node ->
        let sepSpaceAfterFunctionName =
            if node.Arguments.IsNotEmpty then
                sepSpace
            else
    ...
    | Expr.App node ->
        fun ctx ->
            match node with
            | EndsWithDualListApp ctx.Config (sequentialArgs: Expr immarray,
            ...
            | EndsWithSingleListApp ctx.Config (sequentialArgs: Expr immarray, arrayOrList: ExprArrayOrListNode) ->
                let genArrayOrList = genArrayOrList false
            ...
            | EndsWithSingleRecordApp ctx.Config (sequentialArgs: Expr immarray, recordOrAnonRecord) ->
                // check if everything else beside the last array/list fits on one line
                let singleLineTestExpr =
                    genExpr node.FunctionExpr +> sepSpace +> col sepSpace sequentialArgs genExpr
            ...
            | _ ->
                let shortExpression =
                    let sep ctx =
                        match node.Arguments with
                        | EmptyImmutableArray -> ctx
                        | SingleItemImmutableArray singleArg ->
                            sepSpaceBeforeParenInFuncInvocation node.FunctionExpr singleArg ctx
                        | _ -> sepSpace ctx
    ...
    | Expr.IfThenElif node ->
        ...
        let areAllShort ctx =
            let anyThenExprIsIfThenElse =
                node.Branches |> ImmutableArray.exists (fun node -> isIfThenElse node.ThenExpr)
            ...
            let linesToCheck =
                ...
                | None -> ImmutableArray.map checkIfLine node.Branches
                | Some(elseNode, elseExpr) ->
                    ...
                    immarray (1 + node.Branches.Length) {
                        ...
                        for branch in node.Branches do
                            yield checkIfLine branch
                    }

            let lineCheck () =
                linesToCheck
                |> ImmutableArray.forall (fun lineCheck ->
                    let maxWidth =
    ...
    | Expr.DotIndexedGet node ->
        ...
        | Expr.App appNode ->
            match appNode.Arguments with
            | SingleItemImmutableArray(Expr.Constant(Constant.Unit _) as ux) ->
    ...
    if node.Elements.IsEmpty then
        ...
        fun ctx ->
            let alwaysMultiline =
                ...
                ImmutableArray.exists isIfThenElseWithYieldReturn node.Elements
                || ImmutableArray.forall isLambdaOrIfThenElse node.Elements
...
let genTupleMultiline (node: ExprTupleNode) =
    let containsLambdaOrMatchExpr =
        ...
        node.Items
        |> ImmutableArray.chunkBySize 2
        |> ImmutableArray.exists (fun pair ->
            match pair with
            | TwoItemsImmutableArray(Choice1Of2 e, Choice2Of2 _) ->
                match e with
                | Expr.Match _
                | Expr.Lambda _ -> true
                | Expr.InfixApp node ->
                    ...
                    | _ -> false
                | Expr.SameInfixApps node ->
                    match ImmutableArray.last node.SubsequentExpressions with
                    | _, Expr.Lambda _ -> true
                    | _ -> false
                | _ -> false
            | _ -> false)

    let lastIndex = node.Items.Length - 1
...
let genClauses (clauses: MatchClauseNode immarray) =
...
let genExprInMultilineInfixExpr (e: Expr) =
    ...
    | Expr.CompExprBody node ->
        let areLetOrUseStatementsEndingWithOtherStatement =
            let lastIndex = node.Statements.Length - 1

            node.Statements
            |> ImmutableArray.mapi (fun idx statement ->
                match statement with
                | ComputationExpressionStatement.OtherStatement _ -> idx = lastIndex
                | ComputationExpressionStatement.LetOrUseStatement _ -> idx <> lastIndex
                | _ -> false)
            |> Seq.reduce (&&)
...
let genFunctionNameWithMultilineLids (trailing: Context -> Context) (longIdent: IdentListNode) =
    ...
    | HeadAndTailInImmutableArray(IdentifierOrDot.Ident identNode, t) ->
...
let (|EndsWithDualListApp|_|) (config: FormatConfig) (appNode: ExprAppNode) =
    if not (config.ExperimentalElmish || config.IsStroustrupStyle) then
        None
    elif appNode.Arguments.Length < 2 then
        None
    else
        let lastButOne = appNode.Arguments.[appNode.Arguments.Length - 2]
        let last = appNode.Arguments.[appNode.Arguments.Length - 1]

        match lastButOne, last with
        | Expr.ArrayOrList firstList, Expr.ArrayOrList lastList ->
            Some(appNode.Arguments.Slice(0, appNode.Arguments.Length - 2), firstList, lastList)
        | _ -> None

let (|EndsWithSingleListApp|_|) (config: FormatConfig) (appNode: ExprAppNode) =
    if not (config.ExperimentalElmish || config.IsStroustrupStyle) then
        None
    elif appNode.Arguments.Length < 1 then
        None
    else
        let last = appNode.Arguments.[appNode.Arguments.Length - 1]

        match last with
        | Expr.ArrayOrList singleList -> Some(appNode.Arguments.Slice(0, appNode.Arguments.Length - 1), singleList)
        | _ -> None
...
let (|EndsWithSingleRecordApp|_|) (config: FormatConfig) (appNode: ExprAppNode) =
    if not config.IsStroustrupStyle then
        ...
        match last with
        | Expr.Record _
        | Expr.AnonStructRecord _ as singleRecord ->
            Some(appNode.Arguments.Slice(0, appNode.Arguments.Length - 1), singleRecord)
        | _ -> None
...
let genAppWithLambda sep (node: ExprAppWithLambdaNode) =
    let short =
        ...
        +> onlyIf (not node.Arguments.IsEmpty) sepSpace
        +> (genSingleTextNode node.OpeningParen
    ...
    let long (ctx: Context) : Context =
        if ctx.Config.MultiLineLambdaClosingNewline then
            ...
            (genExpr node.FunctionName
             +> ifElse node.Arguments.IsEmpty sep (indent +> sepNln)
             +> genArguments)
                ctx
        else
            match node.Lambda with
            | Choice1Of2 lambdaNode ->
                ...
                let multiLine =
                    ...
                    +> indentSepNlnUnindent (fun ctx ->
                        ...
                        (col sepNln node.Arguments genExpr
                         +> onlyIfNot node.Arguments.IsEmpty sepNln
                         +> genSingleTextNode node.OpeningParen
                         +> (genLambdaWithParen lambdaNode |> genNode lambdaNode)
                         +> addFixedSpaces startColumn
                         +> genSingleTextNode node.ClosingParen)
...
let genTyparDecls (td: TyparDecls) =
    ...
    | TyparDecls.PostfixList node ->
        ...
        +> onlyIf node.Constraints.IsNotEmpty (sepSpace +> genTypeConstraints node.Constraints)
        +> genSingleTextNode node.GreaterThan
        |> genNode node
...
let genPat (p: Pattern) =
    ...
    | Pattern.LongIdent node ->
        ...
        let genParameters =
            ...
            | EmptyImmutableArray -> sepNone
            | SingleItemImmutableArray(Pattern.Paren _ | Pattern.Unit _ as parameter) ->
    ...
    | Pattern.ArrayOrList node ->
        let genPats =
            ...
            let long (ctx: Context) =
                ...
                | SingleItemImmutableArray(Pattern.Or _) ->
...
let genBinding (b: BindingNode) (ctx: Context) : Context =
    let spaceBefore, alternativeSyntax =
        let keywords =
            ImmutableArray.map (fun (st: SingleTextNode) -> st.Text) b.LeadingKeyword.Content

        match keywords with
        | SingleItemImmutableArray "member"
        | SingleItemImmutableArray "override"
        | TwoItemsImmutableArray("static", "member")
        | TwoItemsImmutableArray("abstract", "member")
        | SingleItemImmutableArray "default" ->
    ...
    let isRecursiveLetOrUseFunction =
        b.LeadingKeyword.Content.Length = 1 && b.LeadingKeyword.Content.[0].Text = "and"

    let binding =
        match b.FunctionName with
        | Choice1Of2 functionName when b.Parameters.IsNotEmpty ->
            ...
            let genSignature =
                let spaceBeforeParameters =
                    ...
                    | EmptyImmutableArray -> sepNone
                    | SingleItemImmutableArray p ->
                        let addSpaceBeforeParensInFunDef =
                            match functionName.Content, p with
                            | SingleItemImmutableArray(IdentifierOrDot.Ident newIdent), _ when newIdent.Text = "new" ->
                                false
                            | _, Pattern.Paren _
                            ...
                            | content, _ ->
                                match ImmutableArray.tryLast content with
                                | None -> false
                ...
                let long (ctx: Context) =
                    let genParameters, hasSingleTupledArg =
                        match b.Parameters with
                        | SingleItemImmutableArray(Pattern.Paren parenNode) ->
...
let genBindings withUseConfig (bs: BindingNode immarray) : Context -> Context =
...
let genTypeConstraints (tcs: TypeConstraint immarray) =
...
let genType (t: Type) =
    ...
    | Type.Funs node ->
        ...
        let long =
            if node.Parameters.IsEmpty then
                genType node.ReturnType
            else
                let ht, ha = node.Parameters.[0]
                let rest = node.Parameters.Slice(1, node.Parameters.Length - 1)
    ...
    | Type.AppPrefix node ->
        let addExtraSpace =
            if node.Arguments.IsEmpty then
                ...
                | t -> addSpaceIfSynTypeStaticConstantHasAtSignBeforeString t
...
let genSynTupleTypeSegments (path: Choice<Type, SingleTextNode> immarray) =
...
let genTypeDefn (td: TypeDefn) =
    ...
    match td with
    | TypeDefn.Enum node ->
        let hasMembers = members.IsNotEmpty
    ...
    | TypeDefn.Union node ->
        ...
        let unionCases (ctx: Context) =
            ...
            | EmptyImmutableArray -> ctx
            | SingleItemImmutableArray singleCase when not hasMembers ->
                let hasVerticalBar =
                    ctx.Config.BarBeforeDiscriminatedUnionDeclaration
                    || singleCase.Attributes.IsSome
                    || singleCase.Fields.IsEmpty
    ...
    | TypeDefn.Record node ->
        ...
        let multilineExpression (ctx: Context) =
            ...
            let anyFieldHasXmlDoc =
                Seq.exists (fun (fieldNode: FieldNode) -> fieldNode.XmlDoc.IsSome) node.Fields
...
let genTypeList (node: TypeFunsNode) =
    ...
    let longExpr =
        let rec visit
            (parameters: (Type * 'a) immarray)
            (level: int)
            (continuation: Context -> Context)
            : Context -> Context =
            match parameters with
            | EmptyImmutableArray -> continuation
            | SingleItemImmutableArray(lastType, lastArrow) ->
                continuation
                +> genType lastType
                +> sepSpace
            ...
            | HeadAndTailInImmutableArray((t, arrow), tail) ->
                let isTuple =
                    match t with
                    | Type.Tuple _ -> true
                    | _ -> false
...
let genTypeInSignature (t: Type) =
    ...
    | Type.WithGlobalConstraints node ->
        ...
        | Type.Funs funsNode ->
            let genConstraints =
                let short =
                    ifElse node.TypeConstraints.IsNotEmpty (!- "when ") sepSpace
...
let genUnionCase (hasVerticalBar: bool) (node: UnionCaseNode) =
    ...
    +> atCurrentColumn (
        ...
        +> genOnelinerAttributes node.Attributes
        +> genSingleTextNode node.Identifier
        +> onlyIf node.Fields.IsNotEmpty wordOf
    )
    +> onlyIf node.Fields.IsNotEmpty (expressionFitsOnRestOfLine shortExpr longExpr)
    |> genNode node
...
let genMemberDefnList mds =
    if mds.IsEmpty then
        ...
        colWithNlnWhenMappedNodeIsMultiline false MemberDefn.Node genMemberDefn mds
...
let colWithNlnWhenMappedNodeIsMultiline<'n>
    ...
    : Context -> Context =
    nodes
    |> ImmutableArray.map (fun n -> ColMultilineItem(f n, (mapNode >> sepNlnUnlessContentBefore) n))
...
let genModule (m: ModuleOrNamespaceNode) =
    let newline =
        if m.Declarations.Length = 0 then
            ...
            let h = m.Declarations.[0]
            sepNln +> sepNlnUnlessContentBefore (ModuleDecl.Node h)

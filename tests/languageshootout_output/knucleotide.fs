...
[for len in [1
  2] do yield async { return frequencies dna len }] @ [for str in ["ggt"
  "ggta"
  "ggtatt"
  "ggtattttaatt"
  "ggtattttaatttatagt"] do yield async { return countSubstring dna str }] |> List.rev |> Async.Parallel |> Async.RunSynchronously |> Array.rev |> Seq.concat |> Seq.iter(printfn "%s")
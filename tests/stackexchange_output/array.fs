...
let private reorder cap (array : int[]) = 
  if Array.isEmpty array then array
    ...
    if max = cap then 
      ...
      newArray
    else array |> Array.map(fun (elem) -> if elem = max then max else 1)

let private getZeroes todo = todo |> Seq.takeWhile(fun (elem) -> elem = 0) |> Seq.toArray

let private getNumbers todo cap = todo |> Seq.takeWhile(fun (elem) -> elem <> 0) |> Seq.toArray |> reorder cap

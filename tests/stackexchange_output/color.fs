...
type MyFSColorTable() = 
  ...
  static let colorinfo = loadrgb.Split([|'\010'|]) |> Seq.skip 1 |> Seq.map(fun (line) -> line.Split([|'\009'|])) |> Seq.filter(fun (values) -> values |> Seq.length = 3) |> Seq.map(fun (values) -> (string values.[0], string values.[2])) |> Seq.map(fun (rgb, name) -> (rgb.Split([|' '|]), name)) |> Seq.map(fun (rgb, name) -> [|(name, rgb.[0], rgb.[1], rgb.[2])|])
  ...
  static let Dist (V1 : float[]) V2 = Array.zip V1 V2 |> Array.map(fun (v1, v2) -> pown (v1 - v2) 2) |> Array.sum

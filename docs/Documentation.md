...
fsharp_keep_max_blank_lines=100
...
Existing blank lines are always preserved (via trivia), with exception when [fsharp_keep_max_blank_lines](#fsharp_keep_max_blank_lines) is used.  
...
`{ defaultConfig with KeepMaxBlankLines = 1 }`

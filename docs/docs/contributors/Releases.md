...
Verify that all recent PRs and closed issues are listed in the changelog. Normally, this should ok as we require a changelog entry before we merge a PR.
...
The pipeline does assume that `NUGET_KEY` environment variable is set with a valid NuGet key.

## GitHub release

A new GitHub release entry should be created for official versions. This will notify users that has subscribed to releases.  
In the past some alpha or beta releases have a had a GitHub release, it depends on the occasion.

### Tag

Create a new tag based on the main branch. The assumption is that `CHANGELOG` file contains the tag version you are about to create.
...
For minors and major a codename (inside a `<sub>` tag) is used. All codenames so far have been song titles by the band Ghost (example `# 5.1.0 <sub>Kaisarion</sub> - 11/2022`).  
...
Optionally share (minor or major) release on other social media. 
